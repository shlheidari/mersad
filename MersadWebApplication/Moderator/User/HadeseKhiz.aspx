    ...
    <div id="adminright" class="admin-right-panel" style="overflow-y: auto; float: right !important;">
        ...
        <script>
            ...
            function callApi() {
                ...
                fetch('HadeseKhiz.aspx/GetMessage', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        if (data.d) {
                            printMessageToConsole(data.d);
                        } else {
                            printMessageToConsole("No message received");
                        }
                    })
                    .catch(error => {
            ...
            function testinputs() {
                ...
                var txtinsertelamenoghtesooyePolice = document.getElementById('<%= txtinsertelamenoghtesooyePolice.ClientID %>').value;
                var txtinsertinDatabase = document.getElementById('<%= txtinsertinDatabase.ClientID %>').value;
                var txtinsertdatebazdid = document.getElementById('<%= txtinsertdatebazdid.ClientID %>').value; 
                var txtinsertDategharargiridarsaf = document.getElementById('<%= txtinsertDategharargiridarsaf.ClientID %>').value; 
                var txtinserttarikheshorooeejrayeeghdambarayeNoghte = document.getElementById('<%= txtinserttarikheshorooeejrayeeghdambarayeNoghte.ClientID %>').value; 
                var txtinserttarikhekhatemeyaftanejrayeeghdam = document.getElementById('<%= txtinserttarikhekhatemeyaftanejrayeeghdam.ClientID %>').value; 
                var id = document.getElementById('numberInput2').value;
                var ddlListStatus = document.getElementById('<%= ddlListStatus.ClientID %>').value;
                var txtTozihat = document.getElementById('<%= txtTozihat.ClientID %>').value;
                ...
                const updatedata = {
                    txtinsertelamenoghtesooyePolice: txtinsertelamenoghtesooyePolice,
                    txtinsertinDatabase: txtinsertinDatabase,
                    txtinsertdatebazdid: txtinsertdatebazdid,
                    txtinsertDategharargiridarsaf: txtinsertDategharargiridarsaf,
                    txtinserttarikheshorooeejrayeeghdambarayeNoghte: txtinserttarikheshorooeejrayeeghdambarayeNoghte,
                    txtinserttarikhekhatemeyaftanejrayeeghdam: txtinserttarikhekhatemeyaftanejrayeeghdam,
                    ddlListStatus: ddlListStatus,
                    txtTozihat: txtTozihat,
                    id: id
                };
            ...
            <div class="white-box">
                ...
                <div class="row form-group">
                    <div class="col-6">
                        ...
                        <label for="rbbazeZamani">بازه زمانی</label>
                    </div>
        ...
        <button type="button" onclick="GetSlidShow()" style="margin: 20px 50px 0px 0px; padding: 10px;" title="بررسی">نمایش نقاط</button>
                    ...
                    <input type="text" id="numberInput" style="margin: 20px 50px 0px 0px; padding: 10px;" title="Enter a number" />
    ...
    <div id="mainContainer" class="map-left-panel">
        ...
        <script>
            ...
            function GetDisplay(di) {
                ...
                var textbox = document.getElementById('numberInput2');

                // Set the value of the textbox
                textbox.value = di;
                //console.log("lllll"+di+"sssssssss")
                $.ajax({
                    type: "POST",
        ...
        <a href="AddNewLocation.aspx" target="_parent">
            <img id="addNewa" src="../../images/addlocation.png" style="position: absolute; width: 206px; height: 83px; z-index: 100;" />
                ...
                <a href="UpdateLocation.aspx" target="_parent">
            <img id="updatea" src="../../images/question.png" style="position: absolute; width: 206px; height: 83px; z-index: 100;" />
        </a>
    ...
    <div runat="server" id="PanelAddLocation" style="overflow: scroll; max-height:100%; z-index: 1000; display: none; position: absolute; background-color: white; padding-bottom: 20px; width: 614px; top: 0; left: 0; margin-left: 90px; margin-top: 145px;">
        <asp:UpdatePanel ID="UpdatePanel1" runat="server">
            <ContentTemplate>
                ...
                <label id="smal"></label>
                <asp:Button ID="btnRef" runat="server" Style="visibility: hidden;" OnClick="btnRef_Click" />
                    ...
                    <div class="container">
                                                ...
                                                <ItemTemplate>
                                                    <div class="col-4" style="margin: 10px; float: right; width: 137px; height: 97px; background-image: url(../../images/wordIcon.png);" runat="server" visible='<%# Eval("DataType").Equals(".docx")||Eval("DataType").Equals(".doc") %>'>
                                                        <asp:ImageButton runat="server" Width="50" Height="45" CommandArgument='<%# Eval("id") %>' CommandName="Delete" ImageUrl="~/Images/DeleteImage.jpg" Style="border-radius: 1px !important;" />
                                                               <a href='<%# "../../temp/" + Eval("addressFile") %>' download="filename">
                                                                <img src='<%# "../../temp/" + Eval("addressFile") %>' style="margin: 10px; float: right; width: 137px; height: 97px;" />
                                                                <button style="margin: 10px; float: right;">Downloaddddd</button>
                                                            </a>
                        ...
                        <div class="row align-content-end justify-content-end" style="padding: 10px; margin: 10px; max-height: 80px; height: 80px; text-align: left;">
                            <asp:Button ID="Insertlocationtion" runat="server" CssClass="btn-login" type="button" Text="ثبت" Style="width: 60px; height: 40px; text-align: center;" OnClick="UpdateLocation_Click" />
                                     <button type="button" onclick="testinputs()" style="margin: 20px 50px 0px 0px; padding: 10px;" title="بررسی">نمایش نقccscsccscscاط</button>
                                    <div id="responseDiv" style="margin-top: 20px; padding: 10px; border: 1px solid #ccc;"></div>
