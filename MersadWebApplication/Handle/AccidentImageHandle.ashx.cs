...
{
    /// <summary>
    /// Summary description for AccidentImageHandle
    /// </summary>
    public class AccidentImageHandle : IHttpHandler
    {
        // Token: 0x0600035C RID: 860 RVA: 0x00029CC8 File Offset: 0x00027EC8
        public void ProcessRequest(HttpContext context)
        {
                ...
                if (flag)
                {
                    ...
                    if (flag2)
                    {
                        ...
                        if (flag3)
                        {
                            ...
                            this._sqlHelper.SetDeleteAccidentDiagramImage(guid);
                        }
                        else
                        {
                            foreach (object obj in context.Request.Files)
                            {
                                ...
                                if (flag5)
                                {
                                    ...
                                    if (flag6)
                                    {
                                        string pathToSave2 = HttpContext.Current.Server.MapPath("~/MediaUploader/Accident/");
                                        file.SaveAs(pathToSave2 + guid);
                                        string id = arrQueryStrings[2].Split(new char[]
                                        {
                                            '='
                                        })[1];
                                        this._sqlHelper.SetUpdateAccidentDiagramImage(Convert.ToInt64(id), guid);
                                    }
                                }
                            }
            ...
            catch (Exception ex)
            {
                ...
                string formUrl = "/Handle/AccidentImageHandle";
                Exception ex2 = ex;
        ...
        // (get) Token: 0x0600035D RID: 861 RVA: 0x00029A44 File Offset: 0x00027C44
